...
namespace ConsumerPedidos.src.RabbitMQWorker.Services
{
    public class LeituraMensagem : ILeituraMensagem
    {
        ...
        public void LerMensagem(string message)
        {
            ...
            {
                throw new FormatException("A mensagem enviada está fora do padrão esperado");
            }
            catch (Exception ex)
            {
                throw new Exception($"Houve um erro no formato da mensagem. Segue mensagem de erro: {ex.Message}");
            }
        }
    }
}
